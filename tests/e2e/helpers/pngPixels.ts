...
/** Decodes an 8-bit RGB/RGBA PNG (what Playwright's `screenshot()` returns) into RGB rows. */
export function decodePng(buf: Buffer): { width: number; height: number; pixel: (x: number, y: number) => { r: number; g: number; b: number } } {
  let off = 8;
  let width = 0;
  let height = 0;
  let bpp = 0;
